...
{
    [Authorize]
    public class NewAddModel : PageModel
    {
        private readonly IUserService _userService;
        private readonly INewService _newService;
        public NewAddModel(IUserService userService, INewService newService)
        {
        ...
        public IActionResult OnPost(IFormFile newImage)
        {
            ...
            bool AddnewList = _newService.AddNewList(newList);

            if (AddnewList == true)
            {
                TempData["add_success"] = true;
                return RedirectToPage("newListList_m");
            }
            else
            {
                TempData["add_error"] = true;
                return RedirectToPage("newListAdd");
            }
        }
